...
const Earth: React.FC<EarthProps> = ({ onEarthSceneEnd }) => {
  ...
  // Removed countdownStarted and countdown states

  // Add states for the mission timer
  const [currentUtcTime, setCurrentUtcTime] = useState<Date>(
    new Date(Date.UTC(1969, 6, 16, 13, 27, 38))
  );
  ...
  const [tMinusTime, setTMinusTime] = useState<number>(
    Math.floor(
      (new Date(Date.UTC(1969, 6, 16, 13, 32, 0)).getTime() -
        new Date(Date.UTC(1969, 6, 16, 13, 27, 38)).getTime()) /
        1000
    )
  );
  ...
  const MISSION_START_TIME = new Date(Date.UTC(1969, 6, 16, 13, 27, 38)); // Historical mission start time
  const LAUNCH_TIME = new Date(Date.UTC(1969, 6, 16, 13, 32, 0)); // Actual launch time (T-0)

  // Update UTC time every second and keep T-minus/T-plus in sync
  useEffect(() => {
    // Set initial times based on constants
    setCurrentUtcTime(MISSION_START_TIME);
    setTMinusTime(
      Math.floor((LAUNCH_TIME.getTime() - MISSION_START_TIME.getTime()) / 1000)
    );

    // Create a mission clock that increments from the historical start time
    const missionTimer = setInterval(() => {
      setCurrentUtcTime((prevUtcTime) => {
        const newDate = new Date(prevUtcTime.getTime() + 1000);

        // Calculate time relative to LAUNCH_TIME in seconds
        const timeRelativeToLaunch = Math.floor(
          (LAUNCH_TIME.getTime() - newDate.getTime()) / 1000
        );
        setTMinusTime(timeRelativeToLaunch); // This will become negative after launch
  ...
  useEffect(() => {
    ...
    const stageIntervals = {
      prelaunch: {
        start: Cesium.JulianDate.fromIso8601("1969-07-16T13:27:40Z"),
        ...
        audioSources: [
          {
            ...
            volume: 0.5,
            loop: false,
            startTime: Cesium.JulianDate.fromIso8601("1969-07-16T13:27:40Z"), // T-4min 15sec
          },
          {
            src: "/audio/a11_t-0000135.mp3",
            volume: 0.5,
            loop: false,
            startTime: Cesium.JulianDate.fromIso8601("1969-07-16T13:30:25Z"), // T-1min 35sec
          },
        ],
      },
      stage1: {
              ...
              const onTickListener = () => {
                ...
                if (
                  ...
                  if (
                    ...
                    viewerRef.current.clock.currentTime = oneSecondBefore;
